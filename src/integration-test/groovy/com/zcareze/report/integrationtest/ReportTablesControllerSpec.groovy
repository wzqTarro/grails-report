...
class ReportTablesControllerSpec extends Specification implements ControllerUnitTest<ReportTablesController> {
    ...
    def setup() {
        ...
        new ReportTables(queryMode: 0, rpt: report1, name: "tb0", sqlText: "select name,abbr,gender,birthday,archive_address from resident_list LIMIT 30", seqNum: 2).save()
        new ReportTables(queryMode: 0, rpt: report2, name: "tb0", sqlText: "select A.*,B.name as residentName from rsdt_contract_list as A,resident_list as B where A.resident_id=B.id AND service_months=[month] AND accept_time\n" +
        ...
        new ReportTables(queryMode: 0, name:"createRecord", rpt: report4, seqNum: 1, sqlText: "select ol.`id` as name, COUNT(rl.`id`) as count FROM org_list ol INNER JOIN `resident_list` rl on rl.`org_id`= ol.`id` where ol.`id` = [orgID] and rl.`commit_time` BETWEEN '2019-03-01' and '2019-03-30' GROUP BY ol.`id`").save()
    ...
    void "添加报表数据表-测试1"() {
        given: "参数"
            ...
            ReportTables table = new ReportTables(rpt: report, name: "tb0", sqlText: "select name,abbr,gender,birthday,archive_address from resident_list LIMIT 30", seqNum: 1, queryMode: 0)
        when: "执行"
        ...
        then:"验证结果"
            ...
            assert actualTable.rpt.editTime.after(editTime)
    }

    void "添加报表数据表-测试2"() {
        given: "参数"
        Report report = Report.findByCode("TZYB")
        ReportTables table = new ReportTables(rpt: report, name: "", sqlText: "select name,abbr,gender,birthday,archive_address from resident_list LIMIT 30", seqNum: 1, queryMode: 0)
        when: "执行"
        controller.addReportTable(table)
        then: "结果"
        assert response.status == 200
    ...
    void "添加报表数据表-测试3"() {
        ...
        def jsonData = response.json
        assert jsonData.code == 3
        assert jsonData.message == "[报表数据表]类的属性[queryMode]不能为null;"
    }

    void "添加报表数据表-测试4"() {
        given: "参数"
        Report report = Report.findByCode("TZYB")
        ReportTables table = new ReportTables(rpt: new Report(), name: "tb0", sqlText: "select", seqNum: 1, queryMode: 0)
        when: "执行"
        controller.addReportTable(table)
        then: "结果"
        assert response.status == 200
    ...
    void "添加报表数据表-测试5"() {
        given: "参数"
        Report report = Report.findByCode("TZYB")
        ReportTables table = new ReportTables(rpt: report, name: "tb0", sqlText: "select", seqNum: -1, queryMode: 0)
        when: "执行"
        controller.addReportTable(table)
        then: "结果"
        assert response.status == 200
    ...
    void "添加报表数据表-测试7"() {
        given: "参数"
        Report report = Report.findByCode("TZYB")
        ReportTables table = new ReportTables(rpt: null, name: "tb0", sqlText: "select", seqNum: 1, queryMode: 0)
        when: "执行"
        controller.addReportTable(table)
        then: "结果"
        assert response.status == 200
    ...
    void accessParams(params, param) {
        ...
        params["queryMode"]=param.queryMode
    }
    void "修改报表数据表-测试1"() {
        given:"参数"
            Report report = Report.findByCode("KZRZB")
            def editTime = report.editTime
            def param = [queryMode: 0, rptId: report.id, name: "table", sqlText: "select A.* from org_list as A,(SELECT CODE FROM org_list as B where id=[orgid]) as C where A.code like CONCAT(C.code,'%')", seqNum: 1]
    ...
    void "修改报表数据表-测试2"() {
        ...
        Report report = Report.findByCode("KZRZB")
        def editTime = report.editTime
        def param = [rptId: report.id, name: "table", sqlText: "select", seqNum: 2, queryMode: 0]
        accessParams(controller.params, param)
        when:"执行"
        controller.editReportTable()
        then:"结果"
    ...
    void "修改报表数据表-测试3"() {
        given:"参数"
        Report report = Report.findByCode("KZRZB")
        def param = [rptId: report.id, name: "table", sqlText: "", seqNum: 2, queryMode: 2]
        accessParams(controller.params, param)
        when:"执行"
        controller.editReportTable()
        then:"结果"
        ...
        def jsonData = response.json
        assert jsonData.code == 3
        assert jsonData.message == "[报表数据表]类的属性[queryMode]的值[2]不在列表的取值范围内;"
    }
    void "修改报表数据表-测试3-1"() {
        given:"参数"
        Report report = Report.findByCode("KZRZB")
        def param = [rptId: report.id, name: "table", sqlText: "", seqNum: 2, queryMode: null]
        accessParams(controller.params, param)
        when:"执行"
        controller.editReportTable()
        then:"结果"
    ...
    void "修改报表数据表-测试4"() {
        ...
        Report report = Report.findByCode("KZRZB")
        def editTime = report.editTime
        def param = [rptId: report.id, name: "", sqlText: "select", seqNum: 2]
        accessParams(controller.params, param)
        when:"执行"
        controller.editReportTable()
        then:"结果"
    ...
    void "修改报表数据表-测试5"() {
        ...
        Report report = Report.findByCode("KZRZB")
        def editTime = report.editTime
        def param = [rptId: report.id, name: "table", sqlText: "select", seqNum: -1, queryMode: 0]
        accessParams(controller.params, param)
        when:"执行"
        controller.editReportTable()
        then:"结果"
    ...
    void "修改报表数据表-测试6"() {
        ...
        Report report = Report.findByCode("KZRZB")
        def editTime = report.editTime
        def param = [rptId: report.id, name: "tab", sqlText: "select", seqNum: 2]
        accessParams(controller.params, param)
        when:"执行"
        controller.editReportTable()
        then:"结果"
    ...
    void "修改报表数据表-测试7"() {
        ...
        Report report = Report.findByCode("KZRZB")
        def editTime = report.editTime
        def param = [rptId: null, name: "tab", sqlText: "select", seqNum: 2]
        accessParams(controller.params, param)
        when:"执行"
        controller.editReportTable()
        then:"结果"
    ...
    void "删除表数据表-测试1"() {
        ...
        assert actualReport.editTime.after(editTime)
    }
    void "删除表数据表-测试2"() {
        given:"参数"
        Report report = Report.findByCode("KZRZB")
